...
import com.example.spacedesignsystem.databinding.*
import ge.space.design.DesignSystemComponents
import ge.space.design.main.*
import ge.space.design.main.util.*
import ge.space.ui.util.extension.*

class ShowCaseActivity : BaseActivity(), ShowCaseDisplay {

    private lateinit var binding: ActivitySimpleShowcaseBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySimpleShowcaseBinding.inflate(layoutInflater)
    ...
    private fun setUpToolbar() {
        ...
        binding.toolbarView.navigationIcon?.setColorFilter(
            getColorFromAttribute(R.attr.brand_primary),
    ...
    private fun bindComponentsList(componentSPS: List<ShowCaseComponent>) {
        val listBinding = LayoutSimpleShowcaseListBinding.inflate(
        ...
        val adapter = SimpleListAdapter<ItemComponentBinding, ShowCaseComponent>(
            componentSPS
        ).setup {
            onCreate { parent ->
                ItemComponentBinding.inflate(layoutInflater, parent, false)
            }
            onBind { binding, component, _ ->
                binding.parent.animation = AnimationUtils.loadAnimation(this@ShowCaseActivity, R.anim.list_anim)
            ...
            onClick { _, item, _ ->
                start(this@ShowCaseActivity, item)
            }
        }
    ...
    override fun show(intent: Intent) {
        val actionBinding = LayoutSimpleShowcaseActionBinding.inflate(
            layoutInflater, binding.contentView, true
        )
